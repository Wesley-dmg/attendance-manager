...
# from attendance_api.utils import set_otp_for_user
# from attendance_api.serializers.auth import OTPRequestSerializer, OTPVerifySerializer
...
#                 return Response({"error": "Numéro introuvable ou non autorisé"}, status=404)
...
#                 # ✅ Login réussi, on efface le code
#                 user.otp_code = None
...
#                 user = User.objects.get(phone_number=phone, role="teacher")
#                 now = timezone.now()
#                 if user.otp_code and user.otp_code_expiry and now < user.otp_code_expiry:
#                     otp = user.otp_code  # Réutilise le même
#                 else:
#                     otp = set_otp_for_user(user)
#                 return Response({"message": "OTP renvoyé avec succès"}, status=200)
#             except User.DoesNotExist:
#                 return Response({"error": "Numéro introuvable ou non autorisé"}, status=404)
#         return Response(serializer.errors, status=400)
...
class ResendOTPView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                user = User.objects.get(phone_number=phone, role="teacher")
                if user.otp_code and user.otp_code_expiry and timezone.now() < user.otp_code_expiry:
                    otp = user.otp_code
                else:
                    otp = set_otp_for_user(user)
